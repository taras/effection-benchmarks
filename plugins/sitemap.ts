...
export function sitemapPlugin(): RevolutionPlugin {
  return {
    *http(request, next) {
      const options = yield* useRevolutionOptions();
      const url = new URL(request.url);

      if (url.pathname === "/sitemap.xml") {
        const app = options.app ?? [];
        let paths: RoutePath[] = [];
        for (const middleware of app) {
          const ext = middleware as SitemapExtension;
